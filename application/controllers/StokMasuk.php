...
class StokMasuk extends CI_Controller
{
    ...
    function index(){
        ...
        $config['base_url'] = base_url().'stokMasuk?';   //url yang muncul ketika tombol pada paging diklik
        $config['total_rows'] = $this->MstokMasuk->count_data();
        ...
        //var_dump($this->MstokMasuk->getAll($batas,$offset));die();
        $data['datastokMasuk'] = $this->MstokMasuk->getAll($batas,$offset); 
        $data['data_nolimit'] = $this->MstokMasuk->getAll_nolimit();
        $data['data_lowstok'] = $this->MstokMasuk->getAll_lowstok(); 
        $data['dataproduk'] = $this->Mproduk->getAll(0,0,0);
        $data['datasupplier'] = $this->Msupplier->getAll(0,0,0);
        // var_dump($_POST['barcode_field']);
        $this->template('stokMasuk/vstokMasuk', $data);
    }
    ...
    public function cari()
    {
        ...
        $config['page_query_string'] = TRUE; //mengaktifkan pengambilan method get pada url default
        $config['base_url'] = base_url().'stokMasuk/?key='.$key;   //url yang muncul ketika tombol pada paging diklik
        $config['total_rows'] = $this->MstokMasuk->count_search($search); // jlh total barang
        $config['per_page'] = $batas; //batas sesuai dengan variabel batas
        ...
        $data['datastokMasuk'] = $this->MstokMasuk->get_search($batas,$offset,$search); //query model semua barang
    ...
    function add(){
        $this->template('stokMasuk/vformstokMasuk');
    }

    function add_aksi(){
        $this->form_validation->set_rules('is_unique[stokMasuk.id]');
        ...
        $data = array(
          ...
          'diskon' =>$this->input->post('popc_diskon'),
          'total' =>str_replace(',','',$this->input->post('popc_total')),
          'supplier' =>$this->input->post('popc_supplier'),
          'keterangan' =>$keterangan,
        );

        $this->db->query("UPDATE produk set stok = (stok + ".$this->input->post('popc_jumlah').") where barcode = '".$produk_b."' ");

        $this->MstokMasuk->input_data($data); //akses model untuk menyimpan ke database
        ...
        redirect('stokMasuk'); //jika berhasil maka akan ditampilkan view upload
    }
    ...
    function delete($id){
        ...
        $this->db->query("UPDATE produk set stok = (stok - ".$this->input->post('list_stokmasuk').") where barcode = '".$this->input->post('list_barcode')."' ");
        $this->MstokMasuk->delete_data($where,'stok_masuk');
        redirect($_SERVER['HTTP_REFERER']);
    }

    function edit($id){
        $where = array('id' => $id);
        $data['datastokMasuk'] = $this->MstokMasuk->getWhere(array('id'=>$this->uri->segment(3)))->row_array();
        $this->template('stokMasuk/vformstokMasukedit',$data);
    }
    ...
    function update_aksi(){
        ...
        $data = array(
          ...
          'diskon' =>$this->input->post('pop_diskon'),
          'total' =>str_replace(',','',$this->input->post('pop_total')),
          'supplier' =>$this->input->post('pop_supplier'),
        );
        ...
        $this->MstokMasuk->update_data($where,$data,'stok_masuk'); //akses model untuk menyimpan ke database
    ...
    public function print_pdf(){
        ...
        $data1 = $this->MstokMasuk->print_pdf();
        if ($data1)
        {
            $data['lap_stokMasuk'] = $data1;
        }
        
        ob_start();
        $content = $this->load->view('stokMasuk/lap_stokMasuk',$data);
        ...
        try
        {
            ...
            $html2pdf->Output('Laporan Daftar stokMasuk.pdf');
        }
    ...
    public function cetak_view(){
        ...
        $this->load->view('stokMasuk/lap_stokMasuk',$data); 
    }

    public function cetak_lap_excel(){
        $this->load->model('MstokMasuk');
        $data['lap_stokMasuk'] = $this->MstokMasuk->print_pdf();
        $this->load->view('stokMasuk/lap_stokMasuk_excel',$data); 
    }
