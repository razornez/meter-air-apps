...
class Satuan extends CI_Controller
{
    ...
    function index(){
        ...
        $config['base_url'] = base_url().'satuan?';   //url yang muncul ketika tombol pada paging diklik
        $config['total_rows'] = $this->Msatuan->count_data();
        ...
        $data['datasatuan'] = $this->Msatuan->getAll($batas,$offset); //query model semua barang

        $this->template('satuan/vsatuan', $data);
    }
    ...
    public function cari()
    {
        ...
        $config['page_query_string'] = TRUE; //mengaktifkan pengambilan method get pada url default
        $config['base_url'] = base_url().'satuan/?key='.$key;   //url yang muncul ketika tombol pada paging diklik
        $config['total_rows'] = $this->Msatuan->count_search($search); // jlh total barang
        $config['per_page'] = $batas; //batas sesuai dengan variabel batas
        ...
        $data['datasatuan'] = $this->Msatuan->get_search($batas,$offset,$search); //query model semua barang
    ...
    function add(){
        $this->template('satuan/vformsatuan');
    }

    function add_aksi(){
        $this->form_validation->set_rules('is_unique[satuan.id]');
        ...
        $data = array(
          ...
          'deskripsi' =>$this->input->post('deskripsi'),
            ...
            $this->Msatuan->input_data($data); //akses model untuk menyimpan ke database
            ...
            redirect('satuan'); //jika berhasil maka akan ditampilkan view upload
    }
    ...
    function delete($id){
        ...
        $this->Msatuan->delete_data($where,'satuan');
        redirect($_SERVER['HTTP_REFERER']);
    }

    function edit($id){
        $where = array('id' => $id);
        $data['datasatuan'] = $this->Msatuan->getWhere(array('id'=>$this->uri->segment(3)))->row_array();
        $this->template('satuan/vformsatuanedit',$data);
    }
    ...
    function update_aksi(){
        ...
        $this->Msatuan->update_data($where,$data,'satuan');
    ...
    public function print_pdf(){
        ...
        $data1 = $this->Msatuan->print_pdf();
        if ($data1)
        {
            $data['lap_satuan'] = $data1;
        }
        
        ob_start();
        $content = $this->load->view('satuan/lap_satuan',$data);
        ...
        try
        {
            ...
            $html2pdf->Output('Laporan Daftar satuan.pdf');
        }
    ...
    public function cetak_view(){
        ...
        $this->load->view('satuan/lap_satuan',$data); 
    }

    public function cetak_lap_excel(){
        $this->load->model('Msatuan');
        $data['lap_satuan'] = $this->Msatuan->print_pdf();
        $this->load->view('satuan/lap_satuan_excel',$data); 
    }
